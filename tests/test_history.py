...
def test_save_elapsed_time_at_midnight(mocker):
    ...
    window.timer_label.setText("{:02}:{:02}:{:02}".format(int(hours), int(minutes), int(seconds)))
...
def test_history_view_and_calendar():
    ...
    test_date = datetime.date(current_date.year, current_date.month, test_day).strftime('%Y-%m-%d')
    ...
    QTest.mouseMove(date_label, QPoint(date_label.width() // 2, date_label.height() // 2))
    ...
    elapsed_time = fetch_from_db(test_date)  # replace with the date string for the label you're hovering over
    if elapsed_time:
